...
**Existing databases:** the web app **startup migration** adds missing columns: `application_analytics.ifw_a_ne_count`, `application_analytics.ifw_ctrs_count`, and `applications.continuity_child_of_prior_us`. It also **drops** legacy `application_analytics.billing_attorney_reg` and `billing_attorney_name` if present (those were derived from the first POA attorney in Biblio XML, not true billing data). The CLI **`init`** path alone does not alter existing tables. Optional manual DDL: [`scripts/add_schema_continuity_ctrs.sql`](scripts/add_schema_continuity_ctrs.sql). Run **`analytics`** or portal **Recompute analytics** to populate IFW counts and refresh continuity from XML when present.
