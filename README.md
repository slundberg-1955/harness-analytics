...
**Existing PostgreSQL databases:** after upgrading, run [`scripts/add_ifw_a_ne_count_column.sql`](scripts/add_ifw_a_ne_count_column.sql) once (or rely on a fresh `init` on an empty database). Then run **`analytics`** (or portal **Recompute analytics** per matter) to populate values.
