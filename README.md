...
**Extension-of-time heuristics (estimated, not legal USPTO extension counts):** For each CTNF/CTFR **before the first IFW NOA**, the code assumes a **3 calendar month** response window from that OA’s IFW mail date and finds the first prosecution history event classified as **`RESPONSE_NONFINAL`**, **`RESPONSE_FINAL`**, or **`RCE`** with `transaction_date` strictly after that OA and strictly before the earlier of the **next** OA mail date (if any) and the first NOA mail date. If that response is after the deadline, lateness in whole days is bucketed into **`oa_ext_1mo_count`** (1–30), **`oa_ext_2mo_count`** (31–60), **`oa_ext_3mo_count`** (61–90), and **`oa_ext_gt_90d_count`** (remainder). For each IFW **CTRS** before the first NOA, the assumed window is **2 calendar months**, using the same response-event set and the earlier of the **next** CTRS mail date or first NOA as the upper bound; late buckets are **`ctrs_ext_*`** with the same day bands after that CTRS-specific deadline. Implementation: [`harness_analytics/extension_metrics.py`](harness_analytics/extension_metrics.py).

**Existing databases:** the web app **startup migration** adds missing columns: `application_analytics.ifw_a_ne_count`, `application_analytics.ifw_ctrs_count`, `applications.continuity_child_of_prior_us`, and the eight `oa_ext_*` / `ctrs_ext_*` integer columns. It also **drops** legacy `application_analytics.billing_attorney_reg` and `billing_attorney_name` if present (those were derived from the first POA attorney in Biblio XML, not true billing data). The CLI **`init`** path alone does not alter existing tables. Optional manual DDL: [`scripts/add_schema_continuity_ctrs.sql`](scripts/add_schema_continuity_ctrs.sql), [`scripts/add_extension_columns.sql`](scripts/add_extension_columns.sql). Run **`analytics`** or portal **Recompute analytics** to populate IFW counts, extension heuristics, and refresh continuity from XML when present.
